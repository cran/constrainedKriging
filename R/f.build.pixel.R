...
{  
  ...
  ## hŠtte die untere linke Ecke nicht die gleiche Wahrscheinlichkeit ein Pixel
  ##punkt zu besitzten
  
  t.grid.bbox[1,1] <- t.grid.bbox[1,1] - 0.5 * t.colwidth
  t.grid.bbox[2,1] <- t.grid.bbox[2,1] - 0.5 * t.rowwidth
  
  ## Zufälliger Startpunkt des Gitters im linken unteren Ecke falls ein seed gesetzt wurde
  ...
  ##no.pix.in.poly anzahl pixelcenter in den Polygonflächen
...
for(i in 1:t.n.pred.loc)
   {
	...
	#Liste mit den gemeinsamen FlŠchen
