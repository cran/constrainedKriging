...
###
###				   
### output: C = n x m Kovarianzmatrix (n = Anzahl StŸtzpunkte, m = Anzahl Polygone)
### author: Ch. Hofer
### date: 9-2-2010
{
...
## t.n.iter Anzahl Konfigurationen Ÿber welche die Punkt Block Kovarianz gemittelt wird
...
#	pix.in.poly = n X m binŠre Zugehšrigkeits-Matrix Element ith-Zeile, jth Spalte = 1 bedeutet: iter-Pixelschwerpunkt liegt in jtem Polygon, 0 sonst
	pix.in.poly <- as.matrix( pixconfig$pix.in.poly[ , t.col.range ], ncol = t.n.poly )

	sa.polygons <- pixconfig$sa.polygons[ t.col.range ]
	# Anzal Polygone mit einer kleineren FlŠche als die FlŠche eines Pixels
	t.poly.as.point <- sum( sa.polygons )
#	
	polygon.centroids <- pixconfig$polygon.centroids

#	# StŸtzpunkte-Polygon-Kovarianzmatrix mit NA gefŸllt
	t.pb.cov.matrix <- matrix(nrow = nrow( locations ), ncol = t.n.poly )

#
# 	# Berechung der Punkt-Punkt Kovarianz falls die Polygon-FlŠche kleiner ist als die PixelflŠche
#
...
#
# 	# Berechung der Punkt-Polygon Kovarianz falls die Polygon-FlŠche gršsser ist als die PixelflŠche
#
	if( t.poly.as.point < t.n.poly)
#	# fŸr die Polygon welche durch Pixel approximiert werden
	...
	t.pb.pixel.cov <- lapply(t.poly.with.pixel.list,
    			function(x, locations, t.pixel.center, pixconfig, model)
			{
			    ...
			    t.locations.pix.cov <- f.point.rec.covmat(
							...
							rxwidth  = pixconfig$rowwidth,
   				 			rywidth  = pixconfig$colwidth,
   			 	 			subdivisions = 100,
   				 			param = model
						    )$result
