...
# 2023-12-12 A. Papritz optional parallelization, new arguments ncores and fork
{
  ...
  if( !fork && ncores > 1L ){
    ...
    junk <- clusterExport(
      clstr,
      c(
        "object", "t.pred.designmat", "locations", "model.me.free",
        "t.ichol", "t.isupport.covmat", "t.beta.coef", "t.cov.beta.coef",
        "t.orth.resid", "t.orth.designmat", "t.iR", "method"
      ),
      envir = environment()
    )
  ...
  # auxiliary function to compute the prediction results for one polygon
  # neighbourhood configuration

  f.aux <- function(
    i, pixconfig, t.bb.covmat.list, t.pred.designmat,
  ...
  if(  ex.out == TRUE)
  {
    if( method == 1 | method == 2 )
    {
      ...
      {
        sk.weights.matrix <- t(matrix( unlist( lapply( t.krige.res, function( poly ){ return( poly$sk.weights ) } ) ), ncol = nrow( data ), byrow = TRUE) )
      }else{
        sk.weights.matrix <- t(matrix( unlist( lapply( t.krige.res, function( poly ){ return( poly$sk.weights[1,] ) } ) ), ncol = nrow( data ), byrow = TRUE ) )
      }
      ## extended output is a list
      object <- SpatialPolygonsDataFrame( SpatialPolygons( object@polygons ), data = krige.result, match.ID = FALSE)
